...
class HomeController @Inject()(@Named("userParentActor") userParentActor: ActorRef,
                              ...
                              (implicit ec: ExecutionContext) extends AbstractController(cc) with SimpleOriginCheck {
  ...
  def stockquote() = Action { implicit request: Request[AnyContent] =>
    Ok(views.html.stockquote())
  }
